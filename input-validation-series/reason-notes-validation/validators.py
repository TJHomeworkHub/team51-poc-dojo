...
_ALLOWED_PUNCT = r"\.\,\!\?\'\"\-\(\)\/\:"
_ALLOW_RE = re.compile(rf"^[A-Za-z0-9\s{_ALLOWED_PUNCT}]+$")
_SQL_MARKERS_RE = re.compile(
    r"(--|;|\bUNION\b|\bSELECT\b|\bINSERT\b|\bUPDATE\b|\bDELETE\b|\bDROP\b|\bTABLE\b|\bTRUNCATE\b|/\*)",
    flags=re.IGNORECASE,
)
_SQL_TAUTOLOGY_RE = re.compile(r"""('.*?\bOR\b.*?=.*)|(\bOR\b\s+1=1)|('\s*or\s*'1'='1')""", flags=re.IGNORECASE)
_XSS_RE = re.compile(r"[<>]|<\s*script\b", flags=re.IGNORECASE)
_SUSPICIOUS_PUNCT_RE = re.compile(r"([-']{2,}|;{2,}|\"{2,})")


def _is_string(val) -> bool:
    return isinstance(val, str)


def _trimmed(val: str) -> str:
    return val.strip()


def validate_reason(value) -> Tuple[bool, str]:
    """
    Validate the 'reason' field
    """
    if value is None:
        return False, "Reason is required and must be 10-200 characters"
    ...
    txt = _trimmed(value)
    if len(txt) == 0:
        return False, "Reason must not be empty"

    if len(txt) < REASON_MIN:
        return False, f"Reason must be at least {REASON_MIN} characters"

    if len(txt) > REASON_MAX:
        return False, f"Reason must be at most {REASON_MAX} characters"

    if _XSS_RE.search(txt):
        return False, "Reason contains disallowed characters"

    if _SQL_MARKERS_RE.search(txt) or _SQL_TAUTOLOGY_RE.search(txt):
        return False, "Reason contains disallowed characters"

    if _SUSPICIOUS_PUNCT_RE.search(txt):
        return False, "Reason contains invalid punctuation sequences"

    if not _ALLOW_RE.match(txt):
        return False, "Reason contains invalid characters (allowed: letters, numbers, spaces and . , ! ? ' \" - ( ) / : )"

    return True, ""


def validate_notes(value) -> Tuple[bool, str]:
    """
    ...
    if len(txt) > NOTES_MAX:
        return False, f"Notes must be at most {NOTES_MAX} characters"

    if _XSS_RE.search(txt):
        return False, "Notes contain disallowed characters"

    if _SQL_MARKERS_RE.search(txt) or _SQL_TAUTOLOGY_RE.search(txt):
        return False, "Notes contain disallowed patterns"

    if _SUSPICIOUS_PUNCT_RE.search(txt):
        return False, "Notes contain invalid punctuation sequences"

    if not _ALLOW_RE.match(txt):
        return False, "Notes contain invalid characters (allowed: letters, numbers, spaces and . , ! ? ' \" - ( ) / : )"
...
_DISALLOWED_TOKENS_RE = re.compile(r"(am|pm|gmt|utc|z|[+\-]\d{2}:?\d{2}|<|>|script)", flags=re.IGNORECASE)
DEFAULT_BUSINESS_START = dtime(hour=8, minute=0)
DEFAULT_BUSINESS_END = dtime(hour=18, minute=0)
...
def _time_in_business_hours(t: dtime, start: dtime, end: dtime) -> bool:
    """
    Returns True if start <= t < end
    """
    return (t >= start) and (t < end)


def validate_appointment_time(value) -> Tuple[bool, str]:
    """
    Validate appointment_time
    """
    if value is None:
        return False, "Appointment time is required and must be in HH:MM 24-hour format"

    if not isinstance(value, str):
        return False, "Appointment time must be a string in HH:MM 24-hour format"

    s = value.strip()
    if s == "":
        return False, "Appointment time must not be empty and must be in HH:MM format"

    if _DISALLOWED_TOKENS_RE.search(s):
        return False, "Appointment time contains disallowed or ambiguous tokens; use HH:MM 24-hour format"

    parsed = _parse_hhmm(s)
    if parsed is None:
        return False, "Appointment time must be in 24-hour format HH:MM (e.g., 09:00)"

    if not (_time_in_business_hours(parsed, DEFAULT_BUSINESS_START, DEFAULT_BUSINESS_END)):
        return False, "Appointment time must be during business hours (08:00-17:59)"
