...
from validators import validate_patient_name, validate_patient_email, validate_phone_number, validate_appointment_date, validate_appointment_time, validate_reason, validate_notes
...
def create_appointment():
    ...
    # Validate reason using the validators module
    reason = data.get("reason")
    ok, msg = validate_reason(reason)
    if not ok:
        return jsonify({"error": msg}), 400

    # Validate notes using the validators module
    notes = data.get("notes")
    ok, msg = validate_notes(notes)
    if not ok:
        return jsonify({"error": msg}), 400
